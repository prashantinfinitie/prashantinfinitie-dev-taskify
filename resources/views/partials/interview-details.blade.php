                    ...
                    @foreach ($candidate->interviews as $interview)
                        <div class="interview-item mb-4">
                            <div class="d-flex align-items-center mb-3">
                                <div
                                    class="avatar avatar-sm bg-primary d-flex align-items-center justify-content-center me-3">
                                    <i class="bx bx-calendar-check text-white"></i>
                                </div>
                                <div>
                                    <h6 class="mb-0">{{ $interview->title ?? 'Interview' }}</h6>
                                    {{-- @dd($interview->scheduled_at); --}}
                                    <small class="text-muted">
                                        @php
                                            $scheduledDateTime = \Carbon\Carbon::parse($interview->scheduled_at);
                                        @endphp
                                        {{ $scheduledDateTime->format('M d, Y') }} at
                                        {{ $scheduledDateTime->format('h:i A') }}
                                    </small>
                                </div>
                            </div>

                            <div class="border-start ms-2 ps-4">
                                <div class="mb-2">
                                    <span class="badge bg-label-info">
                                        <i class="bx bx-user me-1"></i>
                                        {{ get_label('interviewer', 'Interviewer') }}:
                                        {{ $interview->interviewer->first_name . ' ' . $interview->interviewer->last_name ?? 'Not assigned' }}
                                    </span>

                                    <span
                                        class="badge bg-label-{{ $interview->status == 'completed' ? 'success' : ($interview->status == 'scheduled' ? 'warning' : 'secondary') }} ms-1">
                                ...
                                @if (!empty($interview->notes))
                                    <div class="bg-light rounded-3 mb-3 p-3">
                                        <p class="mb-0"><strong>{{ get_label('notes', 'Notes') }}:</strong>
                                            {{ $interview->notes }}</p>
                                    </div>
                                @endif
                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header" id="feedback-heading-{{ $interview->id }}">
                                                <button class="accordion-button collapsed" type="button"
                                                    ...
                                                    data-bs-target="#feedback-collapse-{{ $interview->id }}"
                                                    aria-expanded="false"
                                                    aria-controls="feedback-collapse-{{ $interview->id }}">
                                                    {{ get_label('feedback', 'Feedback') }}
                                                </button>
                                            </h2>
                                            <div id="feedback-collapse-{{ $interview->id }}"
                                                    ...
                                                    @foreach ($interview->feedback as $feedback)
                                                        <div class="mb-3">
                                                            <div class="d-flex justify-content-between">
                                                                <p class="mb-1">
                                                                    <strong>{{ get_label('rating', 'Rating') }}:</strong>
                                                                    <span
                                                                        class="text-{{ $feedback->rating > 3 ? 'success' : ($feedback->rating > 2 ? 'warning' : 'danger') }}">
                                                                        @for ($i = 1; $i <= 5; $i++)
                                                                            <i
                                                                                class="bx {{ $i <= $feedback->rating ? 'bxs-star' : 'bx-star' }}"></i>
                                                                        @endfor
                                                                    </span>
                                                                </p>
                                                                <small
                                                                    class="text-muted">{{ format_date($feedback->created_at) }}</small>
                                                            </div>
